 ...
 * Batch: 2016 - 2020
 * New Functionalities added by Shubham sir, Sanskar and Priyav Kumar Jha, CSE, MNNIT Allahabad, 2018-2022
 */
class misc{
	protected $sql;
    ...
    public function deregisterEvent($username,$eventID){
        ...
        if($this->sql->countData('scores','username',$username,'eventID',$eventID) > 0){
            ...
            $this->sql->process();
            $this->sql->query = "DELETE from `round_submissions` where participantID = '$username' and eventID = '$eventID'";
            $this->sql->process();
            $result['status'] = 1;
            return $result;
        }
    ...
    public function timeLeftForNextRound($eventID){
	    $this->console("Shubham sir, Sanskar and Priyav is here");
        $eventID = $this->sql->escape($eventID);
        $event = $this->getEventByEventID($eventID);
        if(!isset($event['eventID'])){
